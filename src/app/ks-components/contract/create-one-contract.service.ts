...
import {IToastConfig} from 'shared-ui-components-lib/types/toast';
...
import {
	IContractDiscountRow,
	IContractSeason,
	ILoadOneContractResults,
	ISeasonParams
} from '../../../types/one-contract';
import {IOneContractParams} from '../../../types/one-contract-object';
import {ROUTES_NAMES} from '../../../core/routes-names';
import {IPopupData} from 'shared-ui-components-lib/types/modal';
...
export class CreateOneContractService {
	...
	constructor(public allOneContractService: AllOneContractService, private http: HttpClient,
							public apiService: ApiService, private popupService: PopupService,
	...
	createOrUpdateContract(oneContractObj: IOneContractParams, cb: any, saveCompleted$: Subject<boolean>): Subject<boolean> {
		if (this.oneContractId) {
			// update contract
			this.apiService.getEndpoints(endPoints => {
				this.http.put(endPoints.sourcing + '/' + this.oneContractId, oneContractObj)
					.subscribe(
						() => {
							this.updateAuditDetails(cb, endPoints.sourcing + '/' + this.oneContractId);
							saveCompleted$.next(true);
						},
						() => saveCompleted$.next(false));
			});
		} else {
			// create contract
			this.apiService.getEndpoints(endPoints => {
					this.http.post(endPoints.sourcing + '/', oneContractObj).subscribe((oneContractId: any) => {
							if (!this.oneContractId) {
								this.router.navigate([ROUTES_NAMES.oneContract, ROUTES_NAMES.createOneContract, {oneContractId: oneContractId.id}]);
							}
							this.oneContractId = oneContractId.id;
							this.updateAuditDetails(cb, endPoints.sourcing + '/' + oneContractId.id);
							saveCompleted$.next(true);
						},
						() => saveCompleted$.next(false));
				}
	...
	updateAuditDetails(cb: any, url: string): void {
		if (cb) {
			this.http.get(url).subscribe((oneContractFromServer: IOneContractParams) => {
				cb(oneContractFromServer);
			});
		}
	}
