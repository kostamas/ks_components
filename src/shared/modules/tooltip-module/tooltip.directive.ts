...
export class TooltipDirective implements OnDestroy, AfterViewInit {
  ...
  compileTooltipComponent(): any {
    ...
    if (this.tooltipComponentOutputs) {
      Object.keys(this.tooltipComponentOutputs)
        .forEach(output => this.subscriptionsArr
  ...
  ngOnDestroy(): void {
    this.isOpen = false;
    this.subscriptionsArr.forEach(subscription => subscription.unsubscribe());
