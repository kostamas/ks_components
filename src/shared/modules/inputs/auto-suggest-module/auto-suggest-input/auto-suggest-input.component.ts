...
import {IAutoSuggestItem, IAutoSuggestValidationStatus} from '../../../../types/auto-suggest';
...
export class AutoSuggestInputComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	public results: IAutoSuggestItem[] = [];
	public modal: IModal;
	...
	@Output('onSelectValue') onSelectValue: EventEmitter<IAutoSuggestItem> = new EventEmitter<IAutoSuggestItem>();
	...
	ngAfterViewInit(): void {
		fromEvent(this.inputElement.nativeElement, 'input')
			...
			.subscribe((results: IAutoSuggestItem[]) => {
				if (results.length > 0) {
	...
	checkIfSearchValueInResults(results: IAutoSuggestItem[]): void {
		const searchText = this.searchTextValue.trim();
		results.forEach((res: IAutoSuggestItem) => {
	...
	syncScrollBar(selectedIndex: number, results: IAutoSuggestItem[]): void {
		...
		const scrollPercentage = ((selectedIndex * (resultsElementHeight / (results.length - selectedIndex))) / resultsElementHeight) * 100;
		if (this.openResultsInModal) {
	...
	setValue = (selectedItem: IAutoSuggestItem, closeResults?: boolean) => {
